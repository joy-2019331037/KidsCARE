...
const AdminAdmit=()=>{
    ...
    const loader =async ()=>{
        try{
            ...
            const res = await axios.get("http://localhost:7000/api/get/req",[]);
            console.log(res);
      ...
      const clickHandler=async (request)=>{
        ...
        Swal.fire({
            ...
            preConfirm: async () => {
              ...
              try{
                  const res= await axios.put("http://localhost:7000/api/update/Inreq",{requestId:request.requestId,time:time})
                  console.log(res);
                  Swal.fire({
                      position: 'center',
                      icon: 'success',
                      title: `Request Id ${request.requestId}, kid named ${name} is admitted for ${pac} package`, 
                      text:"Please Refresh"        
                  }
            ...
            requests.map((request) => (
                        request.approved==='n' && (
                            ...
                            <div className="content">
                                ...
                                <button className="admitButton" onClick={() => clickHandler(request)}>Admit</button>
                            </div>
                        </div>
                        )
                ))
